...
const hasUnreadReplies = (ticket) => {
  // Placeholder — replace with real logic when backend supports it
  return false;
};
...
const ViewAllTickets = () => {
  ...
  const uniqueModules = useMemo(() => {
    const mods = tickets.map((t) => `${t.module_code} - ${t.module_name}`).filter(Boolean);
    return [...new Set(mods)];
  }, [tickets]);
  ...
  const filteredTickets = useMemo(() => {
    return tabTickets.filter((ticket) => {
      ...
      return matchesSearch && matchesModule && matchesCategory && matchesUrgency;
    });
  }, [tabTickets, searchQuery, filterModule, filterCategory, filterUrgency]);
    ...
    const sorted = [...filteredTickets].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case "urgency":
          ...
          valA = order[a.urgency] ?? 4;
          valB = order[b.urgency] ?? 4;
          break;
  ...
  if (loading) {
    return (
      <div className={s.loadingPage}>Loading tickets...</div>
    );
  }
          ...
          <div className={s.tableCard}>
            {paginatedTickets.length === 0 ? (
              <div className={s.emptyState}>
                ...
                <p className={s.emptySubtitle}>Try adjusting your filters or search query</p>
              </div>
            ) : (
